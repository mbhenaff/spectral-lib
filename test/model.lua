...
if paths.filep(opt.savePath .. opt.modelFile .. '.model') then
   ...
   print('restarting at epoch ' .. startEpoch)
   ...
   elseif opt.model == 'deep1' then
      ...
      -- conv layer 4
      model:add(nn.SpectralConvolution(opt.batchSize, opt.nhidden, opt.nhidden, pools2:size(2), opt.k, V3, opt.interpScale))
      model:add(nn.Threshold())
